...
| `HONO_HTTP_CLIENTIP_SOURCE`<br>`hono.http.clientIp.source` | no | `auto` | Determines how the HTTP adapter resolves client IP addresses. Supported values are `auto`, `http-headers`, `proxy-protocol`, `remote-address`. `auto` and `http-headers` both inspect the `Forwarded` and `X-Forwarded-For` headers and fall back to the direct remote address if no usable header is present. `proxy-protocol` enables HA Proxy Protocol support and expects incoming connections to carry Proxy Protocol headers from an upstream proxy/load balancer. `remote-address` uses the direct remote address reported by the transport. If set to an unsupported value, the adapter fails to start. |
