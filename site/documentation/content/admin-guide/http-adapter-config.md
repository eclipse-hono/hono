...
| `HONO_HTTP_CLIENTIP_SOURCE`<br>`hono.http.clientIp.source` | no | `remote-address` | Determines how the HTTP adapter resolves client IP addresses. Supported values are `http-headers`, `proxy-protocol`, `remote-address`. `http-headers` inspects the `Forwarded` and `X-Forwarded-For` headers and falls back to the direct remote address if no usable header is present. `proxy-protocol` enables HA Proxy Protocol support and requires incoming connections to carry Proxy Protocol headers from an upstream proxy/load balancer; direct client connections without Proxy Protocol headers are rejected. `remote-address` uses the direct remote address reported by the transport. If set to an unsupported value, the adapter fails to start. |
