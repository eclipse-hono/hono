...
| `HONO_COAP_CLIENTIP_SOURCE`<br>`hono.coap.clientIp.source` | no | `auto` | Determines how the CoAP adapter resolves client IP addresses. Supported values are `auto`, `remote-address`. `auto` and `remote-address` both use the direct remote address reported by the transport. If set to an unsupported value, the adapter fails to start. |
