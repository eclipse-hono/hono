...
| `HONO_COAP_MESSAGETRACINGLOGENABLED`<br>`hono.coap.messageTracingLogEnabled` | no | `false` | If set to `true` the protocol adapter will start to log every incoming and outgoing CoAP messages with tracing information to ease fixing CoAP communication issues (message connection information, request code, confirmable and confirmed details, MID, token, options and status but not the payload). |
