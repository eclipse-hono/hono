...
| `HONO_COAP_CLIENTIP_SOURCE`<br>`hono.coap.clientIp.source` | no | `remote-address` | Determines how the CoAP adapter resolves client IP addresses. Supported values are `remote-address`. `remote-address` uses the direct remote address reported by the transport. If set to an unsupported value, the adapter fails to start. |
