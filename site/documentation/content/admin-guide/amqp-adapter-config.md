...
| `HONO_AMQP_CLIENTIP_SOURCE`<br>`hono.amqp.clientIp.source` | no | `remote-address` | Determines how the AMQP adapter resolves client IP addresses. Supported values are `proxy-protocol`, `remote-address`. `remote-address` uses the direct remote address reported by the transport. `proxy-protocol` enables HA Proxy Protocol support and requires incoming connections to carry Proxy Protocol headers from an upstream proxy/load balancer; direct client connections without Proxy Protocol headers are rejected. If set to an unsupported value, the adapter fails to start. |
