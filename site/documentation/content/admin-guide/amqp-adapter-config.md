...
| `HONO_AMQP_IDLE_TIMEOUT`<br>`--hono.amqp.idleTimeout` | no | `60000` | The time interval (milliseconds) to wait for incoming traffic from a device before the connection should be considered stale and thus being closed. Setting this property to `0` prevents the adapter from detecting and closing stale connections. |
