...
| `HONO_AMQP_CLIENTIP_SOURCE`<br>`hono.amqp.clientIp.source` | no | `auto` | Determines how the AMQP adapter resolves client IP addresses. Supported values are `auto`, `proxy-protocol`, `remote-address`. `auto` and `remote-address` both use the direct remote address reported by the transport. `proxy-protocol` enables HA Proxy Protocol support and expects incoming connections to carry Proxy Protocol headers from an upstream proxy/load balancer. If set to an unsupported value, the adapter fails to start. |
