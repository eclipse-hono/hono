...
means of starting the corresponding [Docker image](https://hub.docker.com/r/eclipsehono/hono-adapter-amqp/)
created from it.
...
| `HONO_AMQP_CLIENTIP_ENABLED`<br>`hono.amqp.clientIp.enabled` | no | `false` | If set to `true` the protocol adapter includes a `client_ip` property in downstream messages which contains the client device's IP address as determined by the strategy set in `HONO_AMQP_CLIENTIP_SOURCE`. |
| `HONO_AMQP_CLIENTIP_SOURCE`<br>`hono.amqp.clientIp.source` | no | `auto` | Determines how the adapter resolves client IP addresses. Supported values are `auto`, `http-headers`, `proxy-protocol`, `remote-address`. For `auto`, HTTP adapters inspect `Forwarded` and `X-Forwarded-For` headers and fall back to the direct remote address if no usable header is present; non-HTTP adapters use the direct remote address reported by the transport. For `http-headers`, HTTP adapters inspect `Forwarded` and `X-Forwarded-For` headers and fall back to the direct remote address if no usable header is present; non-HTTP adapters do not resolve a client IP address when configured with `http-headers`. For `proxy-protocol`, the adapter enables Proxy Protocol support on the server and uses the remote address reported by the transport (as populated by Proxy Protocol). For `remote-address`, the adapter uses the direct remote address reported by the transport. |
