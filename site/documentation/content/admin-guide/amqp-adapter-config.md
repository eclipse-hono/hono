...
| `HONO_AMQP_MAX_FRAME_SIZE`<br>`--hono.amqp.maxFrameSize` | no | `16384` | The maximum size (in bytes) of AMQP frames that the client should accept from the device. When a device sends a bigger frame, the connection will be closed. |
