...
| `HONO_HEALTH_CHECK_PORT`<br>`--hono.healthCheck.port` | no | `8088` | The secure port the HTTPS server, which exposes the service's health check resources, should listen on. The registry will expose a *readiness* probe at URI `/readiness` and a *liveness* probe at URI `/liveness`. |
...
| `HONO_HEALTH_CHECK_INSECURE_PORT`<br>`--hono.healthCheck.insecurePort` | no | `8088` | The insecure port for the HTTP server, which exposes the service's health check resources, should listen on. The registry will expose a *readiness* probe at URI `/readiness` and a *liveness* probe at URI `/liveness`. |
