...
| `HONO_REGISTRY_REST_RESOURCE_ID_REGEX`<br>`--hono.registry.rest.resourceIdRegex` | no | `^[a-zA-Z0-9-_]+$` | The regular expression to use to validate device ID and tenant ID. Please refer to the [java pattern documentation](https://docs.oracle.com/javase/7/docs/api/java/util/regex/Pattern.html). |
