...
| `HONO_HEALTH_CHECK_BIND_ADDRESS`<br>`--hono.healthCheck.bindAddress` | no | - | The IP address of the network interface that the secure port, which exposes the service's health check resources, should be bound to. TThe HTTPS server will only be started if this value is set and the required key material is present in the configuration. |
