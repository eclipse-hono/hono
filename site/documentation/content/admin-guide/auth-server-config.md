...
| `HONO_AUTH_SVC_PERMISSIONS_PATH`<br>`--hono.auth.svc.permissionsPath` | yes | - | The path to the JSON file defining the identities and corresponding authorities on Hono's endpoint resources. For backwards compatibility with previous releases, the path may contain a `file://` prefix. |
