...
| `${PREFIX}_SNI_SERVER_NAME`<br>`--${prefix}.SniServerName` | no | - | Explicitly override the host to use for the TLS SNI server name. If SNI server name is not specified, the host from `${PREFIX}_HOST` will be used with SNI. |
