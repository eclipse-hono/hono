...
| `${PREFIX}_INACTIVE_LINK_TIMEOUT`<br>`--${prefix}.inactiveLinkTimeout` | no | `0` | Sets the amount of time in milliseconds after which a AMQP sender link will be closed when no message has been sent on it. Links will not automatically be closed if the value is `0`. |
