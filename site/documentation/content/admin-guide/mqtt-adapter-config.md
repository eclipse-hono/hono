...
| `HONO_MQTT_SEND_MESSAGE_TO_DEVICE_TIMEOUT`<br>`--hono.mqtt.sendMessageToDeviceTimeout` | no | `100` | The amount of time (milliseconds) after which the sending of a command to a device using QoS 1 is considered to be failed. The value of this variable should be increased in cases where devices are connected over a network with high latency. The default value is set to the value of the `HONO_MESSAGING_SEND_MESSAGE_TIMEOUT` option. |
