...
| `HONO_MQTT_CLIENTIP_SOURCE`<br>`hono.mqtt.clientIp.source` | no | `auto` | Determines how the adapter resolves client IP addresses. Supported values are `auto`, `proxy-protocol`, `remote-address`. `proxy-protocol` requires a load balancer sending Proxy Protocol headers and enables Proxy Protocol support on the server. |
