---
title: "Getting started with Eclipse Hono using AMQP 1.0"
linkTitle: "Getting started with AMQP 1.0"
description: |
...
downstream applications can consume this data via an Apache Qpid&trade; Dispatch Router using Hono's north bound API
regardless of the communication protocols used by the devices.
...
1. follow the [Deployment Guide]({{< relref "/deployment/helm-based-deployment" >}}) in order to install Hono to your
...
application. The client will connect to Hono's north bound [Telemetry]({{< relref "/api/telemetry" >}}) and
[Event API]({{< relref "/api/event" >}})s using the AMQP 1.0 transport protocol, subscribe to all telemetry and event
...
{{% notice info "Service Unavailable" %}}
