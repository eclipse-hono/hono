...
It is not possible to use both topic names `command//+/req/#` and `command//[<gateway-id>]/req/#` at the same time. As
at that case only the last such subscription would be handled from the Command Router.
