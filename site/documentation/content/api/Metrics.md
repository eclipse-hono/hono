...
| *hono.connections.attempts*        | Counter             | *host*, *component-type*, *component-name*, *outcome*                                        | The number of connection attempts to protocol adapters. The outcome can be observed by looking the tag *outcome* which contains the result and the reason. At the moment there's only one possible value: `adapter-connection-limit-exceeded`<br/> **NB** This metric is only supported by protocol adapters that maintain *connection state* with authenticated devices. In particular, the HTTP adapter does not support this metric. |
