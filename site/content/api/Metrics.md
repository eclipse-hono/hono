...
| *counter.hono.connections.anonymous.count*         | *host*, *protocol*               | Current number of connections with anonymous devices. **NB** This metric is only supported by protocol adapters that maintain *connection state* with anonymous devices. In particular, the HTTP adapter does not support this metric. |
