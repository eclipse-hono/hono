...
| `${PREFIX}_CONNECT_TIMEOUT`<br>`--${prefix}.connectTimeout` | no | `5000` | The maximum amount of time (milliseconds) that the client should wait for a TCP/TLS connection to the service to be established. This property can be used to tune the time period to wait according to the network latency involved with the connection between the client and the service. *NB* This value does not limit the amount of time to wait for the service's AMQP <em>open</em> frame to be received. |
