...
| `HONO_MQTT_MAX_CONNECTIONS`<br>`--hono.mqtt.maxConnections` | no | *matching memory* | The number of maximal concurrent connections the protocol adapter should accept. If not set (or set to `0`), the protocol adapter calculates at startup how many connections it can serve with the given amount of memory. |
