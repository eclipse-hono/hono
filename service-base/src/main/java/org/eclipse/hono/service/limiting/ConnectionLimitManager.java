...
/**
 * This class manages a limit of concurrent connections. The limit can be configured in
 * {@link ProtocolAdapterProperties#setMaxConnections(int)}. If no value is configured, it will set a limit based on the
 * the given strategy.
 */
public class ConnectionLimitManager {

    private static final Logger LOG = LoggerFactory.getLogger(ConnectionLimitManager.class);
     ...
     */
    public ConnectionLimitManager(final ConnectionLimitStrategy strategy, final Supplier<Integer> currentConnections,
    ...
    /**
     * Checks if the connection limit is exceeded, based on the configured limit and the the number of current
     * connections.
     *
     * @return True if the current number of connections is equal or above the limit.
     */
