...
then
  ...
  docker secret create -l project=$NS sandbox-device-identities.json $SCRIPTPATH/sandbox-device-identities.json
  ...
  docker service create --detach=true --name init-device-registry-data \
    ...
    --secret sandbox-device-identities.json \
    --secret sandbox-tenants.json \
    --mount type=volume,source=device-registry,target=/var/lib/hono/device-registry \
    --restart-condition=none \
    busybox sh -c 'cp -u /run/secrets/sandbox-credentials.json /var/lib/hono/device-registry/credentials.json; cp -u /run/secrets/sandbox-device-identities.json /var/lib/hono/device-registry/device-identities.json; cp -u /run/secrets/sandbox-tenants.json /var/lib/hono/device-registry/tenants.json'
fi
