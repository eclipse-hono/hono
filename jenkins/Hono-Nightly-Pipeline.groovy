...
def nightlyBuild() {
    stage('Build') {
        withMaven(maven: 'apache-maven-latest', jdk: 'jdk1.8.0-latest', options: [jacocoPublisher(disabled: true), artifactsPublisher(disabled: true)]) {
            ...
            sh 'mvn --projects :hono-service-auth,:hono-service-messaging,:hono-service-device-registry,:hono-adapter-http-vertx,:hono-adapter-mqtt-vertx,:hono-adapter-kura,:hono-example -am deploy -DcreateJavadoc=true -DenableEclipseJarSigner=true'
        }
    }
}