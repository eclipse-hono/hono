    ...
    stage('Deploy') {
        withCredentials([usernamePassword(credentialsId: 'dc987d42-b594-4597-9afc-297a19f82c55', passwordVariable: 'USER_PW', usernameVariable: 'USER_ID')]) {
            configFileProvider(
                ...
                sh "${mvnHome}/bin/mvn -s ${MAVEN_SETTINGS} clean deploy scm:tag -Drevision=${BUILD_NUMBER}-BOSCH -DskipStaging=true -DconnectionUrl='scm:git:https://${USER_ID}:${USER_PW}@products.bosch-si.com/stash/scm/iothub/eclipse-hono.git'"
            }
        }
    }
}