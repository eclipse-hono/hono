...
7. A pair of private/public keys along with a certificate signed with the intermediary CA's key which together assert the identity of the *Tenant Manager server* component. The key file is `tenant-manager-key.pem` (not password protected) and the corresponding certificate is `tenant-manager-cert.pem`. They are both contained in the PKCS12 key store `tenantManagerKeyStore.p12` using `tenantmanagerkeys` as the password.
