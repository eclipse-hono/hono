...
if [ -n "$TAG" ]
then
  for image in hono-adapter-http-vertx hono-adapter-mqtt-vertx hono-adapter-kura hono-adapter-amqp-vertx hono-adapter-coap-vertx hono-service-auth hono-service-device-registry hono-service-messaging 
  do
