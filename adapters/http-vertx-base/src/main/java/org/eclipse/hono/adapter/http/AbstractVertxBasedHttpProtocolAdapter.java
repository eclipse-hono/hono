            ...
            if (contentType == null) {
                ...
                CompositeFuture.all(tokenTracker, tenantConfigTracker, senderTracker, commandConsumerTracker).compose(ok -> {
                    ...
                    if (tenantConfigTracker.result().isAdapterEnabled(getTypeName())) {
                        ...
                        if (qosHeader == null) {
                            return CompositeFuture.all(sender.send(downstreamMessage), responseReady);
                        } else {
                            return CompositeFuture.all(sender.sendAndWaitForOutcome(downstreamMessage), responseReady);
                        }
                    } else {
                    ...
                    ctx.response().exceptionHandler(t -> {
                        LOG.debug("failed to send response for [{}] message from device [tenantId: {}, deviceId: {}]",
