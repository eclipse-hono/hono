...
public class AbstractVertxBasedCoapAdapterTest {
    ...
    /**
     * Verifies that the <em>onStartupSuccess</em> method is invoked if the HTTP server has been started successfully.
     * 
    ...
    public void testStartInvokesOnStartupSuccess(final TestContext ctx) {
        ...
        final AbstractVertxBasedCoapAdapter<CoapAdapterProperties> adapter = getAdapter(server, s -> onStartupSuccess.complete());
    ...
    public void testStartUpFailsIfCredentialsAuthProviderIsNotSet(final TestContext ctx) {
        ...
        // GIVEN an adapter
        final CoapServer server = getCoapServer(false);
        final AbstractVertxBasedCoapAdapter<CoapAdapterProperties> adapter = getAdapter(server,
    ...
    /**
     * Verifies that the adapter fails the upload of an event with a 403 result if the device belongs to a tenant for
    ...
    public void testUploadTelemetryFailsForDisabledTenant() {
        ...
        final CoapServer server = getCoapServer(false);
        final AbstractVertxBasedCoapAdapter<CoapAdapterProperties> adapter = getAdapter(server, null);
        ...
        adapter.uploadTelemetryMessage(coapExchange, authenticatedDevice, authenticatedDevice);
        // tryMessage(ctx, "my-tenant", "the-device", payload, "application/text");

        // THEN the device gets a 403
        verify(coapExchange).respond(same(ResponseCode.INTERNAL_SERVER_ERROR), anyString());
        // and the message has not been forwarded downstream
        verify(sender, never()).send(any(Message.class));
    }

    /**
     * Verifies that the adapter waits for an event being settled and accepted by a downstream peer before responding
     * with a 202 status to the device.
     */
    @Test
    ...
    /**
     * Verifies that the adapter fails the upload of an event with a 400 result if it is rejected by the downstream
     * peer.
     */
    @Test
    ...
    /**
     * Verifies that the adapter does not wait for a telemetry message being settled and accepted by a downstream peer
     * before responding with a 202 status to the device.
     */
    @Test
